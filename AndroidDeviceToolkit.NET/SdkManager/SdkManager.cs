...
{
    /// <summary>
    /// Provides a C# wrapper around the Android SDK Manager (sdkmanager) command-line tool.
    /// Supports listing installed and available SDK packages, installing/uninstalling packages,
    /// accepting licenses, checking for updates, downloading the SDK, and version management.
    /// The sdkmanager is invoked via Java from the SDK's cmdline-tools directory.
    /// </summary>
    /// <example>
    /// Basic usage:
    /// <code>
    /// var sdkManager = new SdkManager(new DirectoryInfo(@"C:\Android\sdk"));
    /// 
    /// // Check SDK Manager version
    /// var version = sdkManager.GetVersion();
    /// Console.WriteLine($"SDK Manager version: {version}");
    /// 
    /// // List all installed and available packages
    /// var list = sdkManager.List();
    /// foreach (var pkg in list.InstalledPackages)
    ///     Console.WriteLine($"Installed: {pkg.Path} v{pkg.Version}");
    /// foreach (var pkg in list.AvailablePackages)
    ///     Console.WriteLine($"Available: {pkg.Path} v{pkg.Version}");
    /// 
    /// // Install packages
    /// sdkManager.Install("platforms;android-33", "build-tools;33.0.0", "system-images;android-33;google_apis;x86_64");
    /// 
    /// // Uninstall packages
    /// sdkManager.Uninstall("platforms;android-30");
    /// 
    /// // Accept all licenses
    /// sdkManager.AcceptLicenses();
    /// 
    /// // Update all packages
    /// sdkManager.UpdateAll();
    /// 
    /// // Download SDK to a custom directory (if not installed)
    /// await sdkManager.DownloadSdk(new DirectoryInfo(@"C:\MyAndroidSdk"));
    /// 
    /// // Use proxy settings
    /// var proxyManager = new SdkManager(proxy: new SdkManager.SdkManagerProxyOptions
    /// {
    ///     ProxyType = SdkManager.SdkManagerProxyType.Http,
    ...
    public partial class SdkManager : SdkTool
    {
        ...
        /// <summary>
        /// Initializes a new instance with automatic SDK home discovery and default settings.
        /// </summary>
        public SdkManager()
        ...
        /// <summary>
        /// Initializes a new instance with the specified SDK home path and configuration options.
        /// </summary>
        /// <param name="androidSdkHome">Path to the Android SDK home directory. Null for auto-discovery.</param>
        /// <param name="channel">The release channel to use (Stable, Beta, Dev, Canary).</param>
        /// <param name="skipVersionCheck">If true, skips the minimum version check before operations.</param>
        /// <param name="includeObsolete">If true, includes obsolete packages in listings.</param>
        /// <param name="proxy">Optional proxy configuration for network operations.</param>
        ...
        /// <summary>
        /// Initializes a new instance with the specified SDK home directory and configuration options.
        /// </summary>
        ...
        /// <summary>
        /// Gets or sets the proxy configuration for SDK Manager network operations.
        /// </summary>
        public SdkManagerProxyOptions Proxy { get; set; }

        /// <summary>
        /// Gets or sets the release channel for package discovery (Stable, Beta, Dev, or Canary).
        /// </summary>
        public SdkChannel Channel { get; set; } = SdkChannel.Stable;

        /// <summary>
        /// Gets or sets whether to skip the sdkmanager version check before operations.
        /// Set to true if you want to bypass the minimum version requirement.
        /// </summary>
        public bool SkipVersionCheck { get; set; }

        /// <summary>
        /// Gets or sets whether to include obsolete packages in listings.
        /// </summary>
        ...
        /// <summary>
        /// Locates the sdkmanager tool within the SDK. Searches cmdline-tools subdirectories first,
        /// then falls back to tools/bin.
        /// </summary>
